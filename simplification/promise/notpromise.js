        ...
        switch (this.state) {
            case PENDING:
                return (newPromise = new Notpromise((resolve, reject) => {
                    // console.log(this.onfulfilledqueue);
...
function tryResolvePromise(promise2, method, param, resolve, reject, status) {
    // 主要是 method(param) 这个地方可能会 throw err，必须调用 reject，不能往上抛 err（为什么？
    try {
        var res = method(param)
    } catch (e) {
        // 为什么完成态下执行不会报错？
        console.log("tryResolvePromise", method, param, status)
        reject(e)
    }
    ...
    try {
        ...
        if (x && (typeof x === 'function' || typeof x === 'object')) { //如果这边不判断 x && 会怎么样？
            ...
            if (typeof then === 'function') {
                // console.log(JSON.stringify(x.then))
                then.call(x,
                    // x.then( //为什么这个不行?
                    y => {
                        if (called) return
